...
public class EvChargingController {
	...
	public Employee areCredentialsMatched(@RequestBody Employee employee) throws EvChargingException {
	...
	public Optional<List<Booking>> viewSlots(@PathVariable("forDate") @DateTimeFormat(pattern = "yyyy-MM-dd")LocalDate forDate,@PathVariable("selectedChargerType") ChargerType selectedChargerType, @PathVariable("campus") String campus,@PathVariable("city") String city) throws EvChargingException {
		return chargingService.searchSlots(forDate, selectedChargerType, campus, city);
	}
	
	@GetMapping("next-available-date/{chargerType}/{date}/{campus}/{city}")
	public LocalDate getNextAvailableBookingDate(@PathVariable("chargerType") ChargerType selectedChargerType,@PathVariable("date") @DateTimeFormat(pattern = "yyyy-MM-dd")LocalDate selectedDate, @PathVariable("campus")String campus, @PathVariable("city")String city)throws EvChargingException {
		return chargingService.getNextAvailableBookingDate(selectedChargerType, selectedDate, campus, city);
	}
	...
	public Optional<List<Station>> getChargingStations() throws EvChargingException {
		return chargingService.getChargersStations();
	}
	
	@GetMapping("/chargers/of-{campus}-{city}")
	public Optional<List<Charger>> getChargersOfStation(@PathVariable("campus")String campus,@PathVariable("city")String city) throws EvChargingException {
		return chargingService.getChargersOfStation(campus, city);
	}
	...
	public Optional<List<Booking>> getEmployeeAllBookings(@PathVariable String empId) throws EvChargingException {
		return chargingService.getEmployeeAllBookings(empId);
	}
	
	@GetMapping("/get/currentBookings/{empId}")
	public Optional<List<Booking>> getEmployeeCurrentBookings(@PathVariable String empId) throws EvChargingException {
